...
__export(src_exports, {
  ...
  getIgSessionId: () => getIgSessionId,
  getInstagramFollowerCount: () => getInstagramFollowerCount,
  getTikTokFollowerCount: () => getTikTokFollowerCount,
  getTwitterFollowerCountWithBrowser: () => getTwitterFollowerCountWithBrowser,
  getTwitterFollowerCountWithEmbedApi: () => getTwitterFollowerCountWithEmbedApi,
  getYoutubeFollowerCount: () => getYoutubeFollowerCount,
  isFetchError: () => isFetchError
});
module.exports = __toCommonJS(src_exports);
init_cjs_shims();
var import_yt_channel_info = require("yt-channel-info");

// src/utils.ts
init_cjs_shims();
var getYoutubeChannelId = (channel) => {
  if (channel.startsWith("https:")) {
    return channel.replace(/^https?:\/\/(www\.)?youtube\.com\/((c|channel|user)\/)?/, "").split("/")[0];
  }
  return channel;
};
...
var fetchEnhanced = async (url, init) => {
  return fetch(url, init).then((res) => {
    if (!res.ok)
      throw new FetchError2(res);
    return res;
  });
};
var isFetchError = (error) => {
  return error instanceof FetchError2;
};

// src/twitter.ts
async function getTwitterFollowerCountWithEmbedApi(username) {
  return fetchEnhanced(`https://cdn.syndication.twimg.com/widgets/followbutton/info.json?screen_names=${username}&v=${Date.now()}`).then((res) => res.json()).then((data) => {
    if (data.length === 0) {
...
var getCsrfToken = async () => {
  ...
  const csrfToken = ((_a4 = setCookieHeader == null ? void 0 : setCookieHeader.match(/csrftoken=(.*?);/)) == null ? void 0 : _a4[1]) || "";
  return csrfToken;
};
var getIgSessionId = async (username, password) => {
  var _a4, _b;
  ...
  const res = await fetchEnhanced("https://www.instagram.com/accounts/login/ajax/", {
    method: "POST",
  ...
  if (authenticated) {
    let session_id = ((_b = (_a4 = res.headers.get("set-cookie")) == null ? void 0 : _a4.match(/sessionid=(.*?);/)) == null ? void 0 : _b[1]) || "";
    return session_id;
  }
  throw new Error("Username or password is incorrect. Please check and try again");
};
async function getIgFollowerCount(username, sessionId) {
  var _a4, _b, _c;
  const res = await fetchEnhanced(`${config.instagram_base_url}/${username}/?__a=1`, {
    headers: buildHeaders({ userAgent: config.mobile, sessionId })
  });
  const data = await res.json();
  return ((_c = (_b = (_a4 = data.graphql) == null ? void 0 : _a4.user) == null ? void 0 : _b.edge_followed_by) == null ? void 0 : _c.count) || 0;
}
...
// src/index.ts
var getFollowerCount = async (options) => {
  ...
  if (options.type === "twitter") {
    return options.browserContext ? getTwitterFollowerCountWithBrowser(options.username, options.browserContext) : getTwitterFollowerCountWithEmbedApi(options.username);
  }
  throw new Error(`Unknown type: ${options.type}`);
};
async function getInstagramFollowerCount(username, sessionId) {
  return getIgFollowerCount(username, sessionId);
}
async function getYoutubeFollowerCount(channel) {
  const info = await (0, import_yt_channel_info.getChannelInfo)({
...
async function getTikTokFollowerCount(username) {
  ...
  const m2 = /"authorStats":{"followerCount":(\d+),/.exec(text);
